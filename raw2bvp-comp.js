...
function hashUint32(x) {
    x = x | 0;
    x = x +  2127912214 + (x <<  12) | 0;
    x = x ^ -949894596  ^ (x >>> 19) | 0;
    x = x +  374761393  + (x <<   5) | 0;
    x = x + -744332180  ^ (x <<   9) | 0;
    x = x + -42973499   + (x <<   3) | 0;
    x = x ^ -1252372727 ^ (x >>> 16) | 0;
    return x;
}

function readUint32(src, srcIndex) {
...
function compress(src) {
    ...
    const dst = new Uint8Array(Math.floor(src.length + (src.length / 255) + 16));
    let [srcIndex, dstIndex] = [0, 0];
    const searchStep = 1;
    let literalStart = srcIndex;

    while (srcIndex + 4 < src.length) {
        // find match
        const data = readUint32(src, srcIndex);
        const hash32 = hashUint32(data);
        const hash16 = ((hash32 >>> 16) ^ hash32) & 0xffff;
        let matchIndex = hashTable[hash16] - 1;
        hashTable[hash16] = srcIndex + 1;

        const matchFound = matchIndex >= 0;
        const matchCorrect = readUint32(src, matchIndex) === data;
        const matchOffset = srcIndex - matchIndex;
        const matchNearEnough = matchOffset < (1 << 16);
        if (!matchFound || !matchCorrect || !matchNearEnough) {
            srcIndex += searchStep;
            continue;
        }

        const literalCount = srcIndex - literalStart;
        const matchStart = matchIndex;
        while (srcIndex < src.length && src[srcIndex] === src[matchIndex]) {
            srcIndex++;
            matchIndex++;
        }
        const matchLength = matchIndex - matchStart;

        // write token
        const tokenLiteralCount = Math.min(literalCount, 0xf);
        const tokenMatchLength = Math.min(matchLength, 0xf);
        const token = (tokenLiteralCount << 4) | tokenMatchLength;
        dst[dstIndex++] = token;

        // write additional literal count bytes
        if (literalCount >= 0xf) {
            let remaining = literalCount - 0xf;
            while (remaining >= 0xff) {
                dst[dstIndex++] = 0xff;
                remaining -= 0xff;
            }
            dst[dstIndex++] = remaining;
        }

        // write literals
        for (let i = 0; i < literalCount; i++) {
            dst[dstIndex++] = src[literalStart++];
        }

        // write match offset
        dst[dstIndex++] = (matchOffset >>> 0) & 0xff;
        dst[dstIndex++] = (matchOffset >>> 8) & 0xff;

        // write additional match length bytes
        if (matchLength >= 0xf) {
            let remaining = matchLength - 0xf;
            while (remaining >= 0xff) {
                dst[dstIndex++] = 0xff;
                remaining -= 0xff;
            }
        ...
        // move anchor
        literalStart = srcIndex;
    }

    // write remaining literals

    // write token
    const literalCount = src.length - literalStart;
    const matchLength = 0;
    const tokenLiteralCount = Math.min(literalCount, 0xf);
    const tokenMatchLength = Math.min(matchLength, 0xf);
    const token = (tokenLiteralCount << 4) | tokenMatchLength;
...
for (const blockIndex of vec.lexi(blockCount)) {
    ...
    if (!blockAlreadyExists) {
        ...
        files.push({
            path: blockURL,
            data: compress(new Uint8Array(block.data)),
        });
    }
